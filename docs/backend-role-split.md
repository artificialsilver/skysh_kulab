...
- 15분마다 DB snapshot이 저장되는지 확인한다.
- 12/24시간 리포트가 DB snapshot 기준으로 생성되는지 확인한다.
...
- fixture indicator 기반 persona rule engine
- fake `MarketSnapshot` 기반 report engine
- FastAPI 조회 API response schema

서로 의존하는 작업:

- 15분 indicator는 Redis bucket schema에 의존한다.
- Persona engine은 indicator field와 Persona enum에 의존한다.
- DB 저장은 `IndicatorSnapshot`과 `PersonaSnapshot`에 의존한다.
- Report API는 DB snapshot schema에 의존한다.
...
- 2번 담당자는 Redis 15분 bucket을 읽어 PersonaSnapshot을 만들 수 있다.
- 3번 담당자는 PersonaSnapshot을 DB에 저장하고 12/24시간 report API를 제공할 수 있다.
