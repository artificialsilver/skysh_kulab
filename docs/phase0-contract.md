...
Redis는 실시간 1분 압축 계층이고, DB는 15분 단위 snapshot 저장소다.
...
DB에 저장된 15분 snapshot을 12시간 또는 24시간 단위로 요약한 사용자 리포트다.
...
- Persona enum은 영어 key를 내부 표준으로 사용한다.
